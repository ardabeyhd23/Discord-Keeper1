...
async function sendShopWebhook(embed: JsonRecord): Promise<void> {
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ username: "FRArda Mağaza", embeds: [embed], allowed_mentions: { parse: [] } }),
    });
...
async function registerCommands(applicationId: string, guildId: string): Promise<void> {
  ...
  try {
    ...
    try {
      const commands = await discordApi(`/applications/${applicationId}/guilds/${guildId}/commands`) as JsonRecord[];
      const ownerOnly = new Set(["shop", "kampanya", "kampanya-bitir", "urun-ekle", "urun-kaldir", "siparis-tamamla", "link-izni"]);
      await Promise.all((commands ?? [])
        .filter((command) => ownerOnly.has(String(command.name)) && command.id)
        .map((command) => discordApi(`/applications/${applicationId}/guilds/${guildId}/commands/${command.id}/permissions`, {
          method: "PUT",
          body: JSON.stringify({ permissions: [{ id: FRARDA_OWNER_ID, type: 2, permission: true }] }),
        })));
    } catch (error) {
      logger.warn({ err: error, guildId }, "FRArda özel komut görünürlük izinleri ayarlanamadı; runtime kilidi aktif");
    }
