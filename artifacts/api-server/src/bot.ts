...
const guildKnowledgeCache = new Map<string, string>();
...
function fallbackClassification(text: string): {
  ...
  const profanity = ["amk", "aq", "siktir", "orospu", "pic", "yarrak", "ibne", "gerizekali"];
...
async function classifyWithGroq(
  ...
  const messageContent: JsonRecord[] = [
    {
      ...
      text: [
        ...
        'Sadece şu JSON formatında cevap ver: {"category":"clean|argo|kufur|kavga","reason":"kısa Türkçe açıklama","matched":"mesajdaki gerçek kelime veya kısa ifade"}',
        ...
        "matched alanına yalnızca mesajın içinde gerçekten bulunan kelime/ifadeyi yaz. clean ise boş bırak.",
  ...
  try {
    ...
    const category = ["clean", "argo", "kufur", "kavga"].includes(String(parsed.category))
    ...
    // Groq tek başına cezalandırma kararı vermesin. İhlal olarak işaretlediği
    // kelime/ifadeyi gerçekten mesajın içinde doğrulayalım. Bu, masum mesajların
    // yanlışlıkla susturulmasını ciddi şekilde azaltır.
    if (category !== "clean" && matched && normalizedContent.includes(matched)) {
      return { category, reason: String(parsed.reason ?? fallback.reason) };
    }

    // Yerel yüksek güvenli filtre zaten ihlal bulduysa onu kullan.
...
async function notifyFrArdaOwner(caseData: ModerationCase): Promise<void> {
  ...
  await sendMessage(channelId, {
    content: `FrArda moderasyon itirazı geldi.\n👤 **Kullanıcı:** ${displayName} (@${username})\n🆔 **ID:** \`${caseData.userId}\`\n<@${caseData.userId}> itiraz etti.${appeal}`,
    embeds: [{ title: "FrArda • Moderasyon İtirazı", description: `👤 Kullanıcı: **${displayName}** (@${username})\n🆔 ID: \`${caseData.userId}\`\n🏷️ Kategori: **${caseData.category}**\n📌 Gerekçe: ${caseData.reason}\n🧾 Vaka: \`${caseData.id}\``, color: 0xeab308, footer: { text: "5 dakika içinde karar verilmezse kullanıcıya kural sınaması gönderilir." } }],
...
async function saveGuildKnowledge(
  ...
  guildKnowledgeCache.set(guildId, knowledge);
}

async function getCachedGuildKnowledge(guildId: string): Promise<string | null> {
  return guildKnowledgeCache.get(guildId) ?? null;
}
