...
function shopOpenButton(): JsonRecord {
  // Activity mode is enabled by default. Set FR_SHOP_ACTIVITY_ENABLED=false
  // to keep the old browser-link behavior.
  const activityEnabled = process.env.FR_SHOP_ACTIVITY_ENABLED?.trim().toLowerCase() !== "false";
  if (activityEnabled) {
    return {
      type: 2,
      style: 1,
  ...
  if (FR_SHOP_ACTIVITY_URL) {
    return { type: 2, style: 5, label: "🛍️ Web Mağazası", url: FR_SHOP_ACTIVITY_URL };
  }
...
async function handleComponent(interaction: JsonRecord): Promise<void> {
  ...
  if (customId === "shop_open") {
    ...
    if (activityEnabled) {
      await discordApi(
        `/interactions/${interaction.id}/${interaction.token}/callback`,
        { method: "POST", body: JSON.stringify({ type: 12 }) },
      );
      return;
    }
