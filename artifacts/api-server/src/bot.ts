...
let shopState: ShopState = { users: {}, coinHistory: [], orders: [], supportTickets: [], lastCampaignAt: {}, campaignDiscount: {}, campaignExpiresAt: {}, shopCatalog: {}, shopMessageIds: {} };
...
export const FRARDA_OWNER_ID = "1231243551053053982";
...
function shopPublicEmbed(coins: number | null = null, guildId = ""): JsonRecord {
  ...
  return {
    title: "🛍️ FR FAMILY SHOP • MAĞAZA",
    description: [
      "**Tarzını yükseltmeye hazır mısın?**",
      "Rolleri, özel ürünleri ve kampanyaları tek ekrandan keşfet.",
      "",
      coins === null ? "💰 **Bakiyeni görmek için:** `/coin` komutunu kullan." : `💰 **Bakiyen:** **${coins.toLocaleString("tr-TR")} Coin**`,
      "",
      "🔐 Siparişler güvenli şekilde incelenir ve onaydan sonra teslim edilir.",
...
async function sendShopWebhookPayload(payload: JsonRecord): Promise<boolean> {
  const rawUrl = process.env.SHOP_WEBHOOK_URL?.trim();
  if (!rawUrl) return false;
  try {
    // Discord webhook'larında message components'in kabul edilmesi için
    // Execute Webhook isteğine with_components=true eklenir.
...
export async function createShopOrder(guildId: string, user: JsonRecord, product: string, price: number): Promise<ShopOrder | null> {
  await loadShopState();
  const userId = String(user.id ?? "");
...
function shopOpenButton(): JsonRecord {
  ...
  // "link" mode is a permission-safe fallback for servers/channels where
  // members do not have Discord's Use Activities permission.
  if (FR_SHOP_ACTIVITY_MODE === "link" && FR_SHOP_ACTIVITY_URL) {
...
async function handleShopSlash(interaction: JsonRecord, guildId: string): Promise<void> {
  ...
  const webhookSent = await sendShopWebhookPayload(shopPayload);
  const sent = webhookSent
    ? { id: `webhook-${Date.now()}` }
    : await sendMessage(String(channel.id), shopPayload);
  ...
  await saveShopState();
  await interactionReply(interaction, `✅ FR Family Shop giriş mesajı #mağaza kanalına ${webhookSent ? "webhook ile" : "bot ile"} gönderildi.`, [], true);
}
...
async function handleSlashCommand(interaction: JsonRecord): Promise<void> {
  ...
  const ownerOnlyCommands = new Set(["shop", "kampanya", "kampanya-bitir", "urun-ekle", "urun-kaldir", "siparis-tamamla", "link-izni"]);
  if (ownerOnlyCommands.has(command) && String(interaction.user?.id ?? interaction.member?.user?.id ?? "") !== FRARDA_OWNER_ID) {
