...
function shopOpenButton(): JsonRecord {
  // Prefer the deployed web shop when an URL is configured. This avoids
  // requiring the Discord application's EMBEDDED flag just to open the shop.
  // Activity mode can be added later once the Discord app is configured for it.
...
async function handleComponent(interaction: JsonRecord): Promise<void> {
  ...
  if (customId === "shop_open") {
    // If a URL is configured, shopOpenButton() already uses a link button.
    // Keep a safe fallback for old messages that still contain custom_id=shop_open.
    if (FR_SHOP_ACTIVITY_URL) {
      await interactionReply(interaction, "🛍️ Web mağaza bağlantısı hazır.", [], true);
      return;
    }
