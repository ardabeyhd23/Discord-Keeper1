  ...
  try {
    ...
    if (names.some((name) => normalizeChannelName(name) === normalizeChannelName("kampanyalar"))) {
      try {
        ...
        if (String(campaignChannel?.guild_id ?? "") === guildId && Number(campaignChannel?.type) === 0) {
...
async function handleShopSlash(interaction: JsonRecord, guildId: string): Promise<void> {
  ...
  const sent = await sendMessage(String(channel.id), {
    ...
    components: [row(button("shop_open", "🛍️ Mağaza Aç", 1))],
    allowed_mentions: { parse: [] },
  });
...
async function sendCampaignMessage(guildId: string, type: "start" | "end"): Promise<boolean> {
  ...
  try {
    ...
    if (String(channel?.guild_id ?? "") === guildId && Number(channel?.type) === 0) {
...
async function handleSlashCommand(interaction: JsonRecord): Promise<void> {
  ...
  if (command === "kampanya") {
    ...
    const campaignMessage = campaignEmbed(guildId, "start");
    if (result.messageSent) {
      await interactionReply(interaction, "🛍️ Kampanya başlatıldı. Aşağıdaki kampanya duyurusu gönderildi ve mağaza GUI'leri yenilendi.", [], false);
      await editInteractionOriginal(interaction, { content: "🛍️ Kampanya başlatıldı. Kampanya duyurusu #kampanyalar kanalına gönderildi. Mağaza GUI'leri yenilendi.", embeds: [campaignMessage], components: [] });
    } else {
      await interactionReply(interaction, "⚠️ Kampanya başlatıldı fakat #kampanyalar kanalına otomatik mesaj gönderilemedi. Aşağıdaki duyuru komutu kullandığın yerde gösteriliyor. Kanal izinlerini kontrol et.", [], false);
      await editInteractionOriginal(interaction, { content: "FR FAMILY SHOP • KAMPANYA (KANAL GÖNDERİMİ BAŞARISIZ)", embeds: [campaignMessage], components: [] });
    }
    return;
  }
  ...
  if (command === "kampanya-bitir") {
    ...
    if (ended) {
      await interactionReply(interaction, "🛍️ Kampanya bitirildi. Fiyatlar normale döndü ve mağaza GUI'leri yenilendi.", [], false);
      await editInteractionOriginal(interaction, { content: "🛍️ Kampanya sona erdi. Aşağıdaki bitiş duyurusu gösteriliyor.", embeds: [campaignEmbed(guildId, "end")], components: [] });
    } else {
