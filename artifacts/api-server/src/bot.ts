...
let shopState: ShopState = { users: {}, orders: [], lastCampaignAt: {}, campaignDiscount: {}, campaignExpiresAt: {} };
...
async function handleShopSlash(interaction: JsonRecord, guildId: string): Promise<void> {
  await loadShopState();
  // /shop kullanıldığında, 5 günlük otomatik dönem geldiyse önce kampanyayı başlatıp
  // #kampanyalar kanalına GUI/embed mesajını gönderir. Aktif kampanyayı yeniden başlatmaz.
...
async function sendCampaignMessage(guildId: string, type: "start" | "end"): Promise<void> {
...
async function startShopCampaign(guildId: string, notify = true): Promise<{ ok: boolean; discount?: number }> {
  await loadShopState();
  ...
  if (notify) await sendCampaignMessage(guildId, "start");
...
async function runCampaignForGuild(guildId: string): Promise<void> {
  ...
  await startShopCampaign(guildId, true);
}
...
async function handleSlashCommand(interaction: JsonRecord): Promise<void> {
  ...
  if (command === "kampanya") {
    ...
    await interactionReply(interaction, `🛍️ Kampanya başlatıldı. **%${result.discount}% indirim** 2 gün boyunca aktif. Kampanya mesajı #kampanyalar kanalına gönderildi.`, [], true);
    return;
  }
  ...
  if (command === "kampanya-bitir") {
    ...
    await interactionReply(interaction, ended
      ? "🛍️ Kampanya bitirildi. Fiyatlar normale döndü ve #kampanyalar kanalına bitiş mesajı gönderildi."
      : "🛍️ Aktif bir kampanya bulunamadı.", [], true);
    return;
  }
...
async function handleComponent(interaction: JsonRecord): Promise<void> {
  ...
  if (customId === "shop_menu") {
    ...
    if (selected === "roles") {
      ...
      const options = roles.map((role) => ({ label: String(role.name).slice(0, 100), value: String(role.id), description: `Fiyat: ${shopEffectivePrice(guildId, shopPriceForRole(role)).toLocaleString("tr-TR")} coin${shopCampaignActive(guildId) ? ` • %${shopCampaignDiscount(guildId)} kampanya` : ""}`.slice(0, 100) }));
