...
const shopStatePath = path.join(process.cwd(), "data", "fr-family-shop.json");
let shopState: ShopState = { users: {}, orders: [], lastCampaignAt: {} };
...
const SHOP_CAMPAIGN_DISCOUNT = 0.5;
...
const SHOP_ROLE_PRICES: Record<string, number> = {
  "FR | EMPEROR": 50_000,
  "FR | KING": 40_000,
  "FR | ROBUX MANYAĞI": 45_000,
  "FR | AYYILDIZ": 20_000,
  "FR | ELITE": 30_000,
  "FR | BAŞ YÖNETİCİ": 15_000,
  "FR | KIDEMLİ AİLE YÖNETİCİSİ": 12_500,
  "FR | BARON": 10_000,
  "FR | VIP": 5_000,
};
...
function isShopRole(role: JsonRecord): boolean {
  ...
  let configuredById = false;
  try {
    const configured = JSON.parse(process.env.SHOP_ROLE_PRICES_JSON ?? "{}") as Record<string, unknown>;
    configuredById = Number(configured[String(role.id)] ?? 0) > 0;
  } catch { /* varsayılan liste kullan */ }
  // Sadece gerçekten satılmak üzere tanımlanmış roller mağazada görünür.
  // Böylece FR | BOT gibi teknik/işlevsel roller asla satış listesine girmez.
  return configuredById || Object.prototype.hasOwnProperty.call(SHOP_ROLE_PRICES, name);
}
...
async function loadShopState(): Promise<void> {
  ...
  try {
    ...
    // İlk çalıştırma veya Render'da boş disk: temiz başlangıç.
  }
}

async function saveShopState(): Promise<void> {
  try {
    await mkdir(path.dirname(shopStatePath), { recursive: true });
    await writeFile(shopStatePath, JSON.stringify(shopState, null, 2), "utf8");
  } catch (error) {
    logger.warn({ err: error }, "FR Family Shop verileri kaydedilemedi");
  }
}
...
async function setShopCoins(guildId: string, userId: string, coins: number): Promise<void> {
  shopState.users[shopUserKey(guildId, userId)] = { coins: Math.max(0, Math.floor(coins)) };
  await saveShopState();
}

function shopPriceForRole(role: JsonRecord): number {
  try {
    const configured = JSON.parse(process.env.SHOP_ROLE_PRICES_JSON ?? "{}") as Record<string, unknown>;
...
function shopCampaignActive(guildId: string): boolean {
  ...
  return Boolean(last && Date.now() - last < SHOP_CAMPAIGN_INTERVAL_MS);
}

function shopEffectivePrice(guildId: string, basePrice: number): number {
  return shopCampaignActive(guildId) ? Math.max(1, Math.floor(basePrice * SHOP_CAMPAIGN_DISCOUNT)) : basePrice;
}
...
async function findNamedChannel(guildId: string, names: string[]): Promise<JsonRecord | null> {
  try {
    ...
    return list.find((c) => Number(c.type) === 0 && wanted.has(normalizeChannelName(String(c.name ?? "")))) ?? null;
  } catch { return null; }
}
...
function shopPublicEmbed(guildId: string): JsonRecord {
  const campaign = shopCampaignActive(guildId);
  return {
    title: "👑 FR FAMILY SHOP 🛒",
    description: [
      "**FR Family Shop'a hoş geldin!**",
      "Aşağıdaki **Mağazayı Aç** butonuna basınca sana özel mağaza arayüzü açılır.",
      "💰 Coin bakiyen ve sipariş ekranların **sadece sana görünür**; başka kullanıcıların bakiyesi değişmez.",
      campaign ? "🔥 **KAMPANYA AKTİF:** 5 gün boyunca mağazadaki ürünlerde **%50 indirim** uygulanıyor." : "",
      "",
      "🛡️ **Güvenli alışveriş:** Siparişler otomatik teslim edilmez; yönetici onayından sonra işleme alınır.",
    ].filter(Boolean).join("\n"),
    color: 0x6d4aff,
    image: { url: shopBannerUrl() },
    fields: [
      { name: "👑 Üst Roller", value: "Rol seç • fiyatı gör • onay ekranına geç", inline: true },
      { name: "🎨 Özel Rol", value: "Kendine özel rol talebi oluştur", inline: true },
      { name: "🎁 Diğer Ürünler", value: "Nitro ve diğer ürünler", inline: true },
      { name: "💰 Coin Satın Al", value: "Coin işlemleri için FRArda ile iletişim", inline: true },
      ...(campaign ? [{ name: "🔥 Kampanya", value: "%50 indirim • 5 gün", inline: true }] : []),
    ],
    footer: { text: "FR FAMILY SHOP • Her kullanıcıya özel arayüz • Siparişler manuel onaylanır" },
    timestamp: new Date().toISOString(),
  };
}

function shopPersonalHomeEmbed(guildId: string, user: JsonRecord, coins: number): JsonRecord {
  const campaign = shopCampaignActive(guildId);
  const displayName = String(user.global_name ?? user.username ?? "Kullanıcı");
  return {
    title: "👑 FR FAMILY SHOP 🛒",
    description: [
      `👋 **Hoş geldin, ${displayName}!**`,
      `💰 **Bakiyen:** **${coins.toLocaleString("tr-TR")} Coin**`,
      campaign ? "🔥 **KAMPANYA AKTİF:** Tüm mağaza ürünlerinde **%50 indirim**." : "🛍️ Güncel ürünleri aşağıdaki menüden seçebilirsin.",
      "",
      "🔒 Bu menü **sana özeldir**. Buradaki bakiye, ürünler ve sipariş ekranları diğer üyelerde görünmez.",
    ].join("\n"),
    color: 0x6d4aff,
    image: { url: shopBannerUrl() },
    fields: [
      { name: "👑 Üst Roller", value: "Sunucudaki satışa açık FR rollerini incele.", inline: true },
      { name: "🎨 Özel Rol", value: `${shopEffectivePrice(guildId, SHOP_SPECIAL_ROLE_PRICE).toLocaleString("tr-TR")} Coin • kişisel rol talebi`, inline: true },
      { name: "🎁 Diğer Ürünler", value: `${shopEffectivePrice(guildId, SHOP_NITRO_PRICE).toLocaleString("tr-TR")} Coin • 1 Aylık Discord Nitro`, inline: true },
      { name: "💰 Coin Satın Al", value: "Coin almak için FRArda ile iletişime geç.", inline: true },
      ...(campaign ? [{ name: "🏷️ İndirim", value: "Eski fiyatlar üzeri çizili, %50 indirimli fiyatlar otomatik gösterilir.", inline: false }] : []),
    ],
    footer: { text: "FR FAMILY SHOP • Kişisel mağaza arayüzü" },
    timestamp: new Date().toISOString(),
  };
}

function shopRoleEmoji(roleName: string): string {
  const name = normalizedShopRoleName(roleName);
  if (name.includes("EMPEROR")) return "👑";
  if (name.includes("KING")) return "💎";
  if (name.includes("ROBUX MANYAĞI")) return "💰";
  if (name.includes("AYYILDIZ")) return "🇹🇷";
  if (name.includes("ELITE")) return "⭐";
  if (name.includes("BAŞ YÖNETİCİ")) return "🛡️";
  if (name.includes("KIDEMLİ AİLE YÖNETİCİSİ")) return "🏅";
  if (name.includes("BARON")) return "🏰";
  if (name.includes("VIP")) return "💠";
  return "🎖️";
}

function shopRoleIconUrl(role: JsonRecord): string | undefined {
  const roleId = String(role.id ?? "");
  const icon = String(role.icon ?? "");
  if (!roleId || !icon) return undefined;
  return `https://cdn.discordapp.com/role-icons/${roleId}/${icon}.png?size=64`;
}

function shopRoleEmbeds(roles: JsonRecord[], guildId: string, coins: number): JsonRecord[] {
  const campaign = shopCampaignActive(guildId);
  const header: JsonRecord = {
    title: "👑 FR FAMILY SHOP • ÜST ROLLER",
    description: [
      `💰 **Bakiyen:** ${coins.toLocaleString("tr-TR")} Coin`,
      campaign ? "🔥 **Kampanya aktif:** mağazadaki ürünlerde **%50 indirim**" : "",
      "",
      "Her rol kendi Discord rol görseliyle gösterilir. Aşağıdaki menüden rolü seç; satın almadan önce sana **onay ekranı** gösterilecek.",
  ...
  const roleEmbeds = roles.slice(0, 9).map((role) => {
    const base = shopPriceForRole(role);
    const price = shopEffectivePrice(guildId, base);
    const icon = shopRoleIconUrl(role);
    const unicodeEmoji = String(role.unicode_emoji ?? "").trim();
    return {
      title: `${unicodeEmoji || shopRoleEmoji(String(role.name))} ${String(role.name)}`,
      description: campaign
        ? `💰 **${price.toLocaleString("tr-TR")} Coin**\n~~${base.toLocaleString("tr-TR")} Coin~~ • 🔥 **%50 indirim**`
        : `💰 **${price.toLocaleString("tr-TR")} Coin**`,
      color: 0x6d4aff,
      ...(icon ? { thumbnail: { url: icon } } : {}),
      fields: [
        { name: "🪙 Bakiyen", value: `${coins.toLocaleString("tr-TR")} Coin`, inline: true },
        { name: "🛒 Satın Alma", value: "Rolü seçtiğinde önce onay ekranı açılır.", inline: true },
      ],
      footer: { text: "FR Family Shop • Rol teslimi yönetici onayından sonra yapılır" },
    };
  });
  // Discord bir mesajda en fazla 10 embed kabul eder: başlık + 9 rol.
  return [header, ...roleEmbeds];
}
...
function shopRolesEmbed(roles: JsonRecord[], guildId: string, coins: number): JsonRecord {
  ...
  return {
    ...
    fields: roles.slice(0, 25).map((role) => {
      ...
      const price = shopEffectivePrice(guildId, base);
      return {
        name: `${shopRoleEmoji(String(role.name))} ${String(role.name)}`,
        value: `**${price.toLocaleString("tr-TR")} Coin**${campaign ? `\n~~${base.toLocaleString("tr-TR")}~~ • %50 indirim` : ""}`,
        inline: true,
      };
    }),
    footer: { text: "FR Family Shop • Discord rol sırası korunur" },
...
function shopMenu(): JsonRecord {
  return {
    ...
    options: [
      { label: "Üst Roller", value: "roles", description: "👑 Satışa açık FR rollerini görüntüle", emoji: { name: "👑" } },
      ...
      { label: "Diğer Ürünler", value: "other", description: "🎁 Nitro ve diğer ürünleri görüntüle", emoji: { name: "🎁" } },
    ],
  };
}

function shopOpenButton(): JsonRecord {
  return row(button("shop_open", "🛒 Mağazayı Aç", 1));
}
...
async function createShopOrder(guildId: string, user: JsonRecord, product: string, price: number): Promise<ShopOrder | null> {
  ...
  await setShopCoins(guildId, userId, coins - price);
...
async function publishShopMenu(guildId: string): Promise<boolean> {
  await loadShopState();
  // /shop veya düz `shop` kullanıldığında kampanya zamanı geldiyse #kampanyalar'a mesaj gider.
  await runCampaignForGuild(guildId);
  const channel = await findNamedChannel(guildId, ["mağaza", "magaza", "shop"]);
  if (!channel?.id) return false;
  await sendMessage(String(channel.id), {
    embeds: [shopPublicEmbed(guildId)],
    components: [shopOpenButton()],
    allowed_mentions: { parse: [] },
  });
  return true;
}
...
async function handleShopSlash(interaction: JsonRecord, guildId: string): Promise<void> {
  const published = await publishShopMenu(guildId);
  await interactionReply(
    interaction,
    published
      ? "✅ FR Family Shop menüsü #mağaza kanalına gönderildi. Kampanya zamanı geldiyse #kampanyalar kanalına da otomatik kampanya mesajı gönderildi."
      : "❌ `#mağaza` kanalı bulunamadı.",
    [],
    true,
  );
}
...
async function runCampaignForGuild(guildId: string): Promise<void> {
  ...
  await sendMessage(String(channel.id), {
    embeds: [{
      title: "🎉 FR Family Shop • Yeni Kampanya",
      description: "🔥 Yeni kampanya başladı! 5 gün boyunca FR Family Shop'taki ürünlerde **%50 indirim** uygulanır. İndirimli fiyatlar mağazada otomatik olarak gösterilir.",
      color: 0xf59e0b,
      image: process.env.SHOP_CAMPAIGN_IMAGE_URL ? { url: process.env.SHOP_CAMPAIGN_IMAGE_URL } : undefined,
      fields: [
        { name: "🏷️ Kampanya", value: "Mağazadaki ürünlerde **%50 indirim**", inline: true },
        { name: "⏳ Dönem", value: "5 gün", inline: true },
        { name: "🛍️ Nereden?", value: "#mağaza", inline: true },
      ],
      footer: { text: "FR Family Shop • Kampanya otomatik başlatıldı" },
      timestamp: new Date().toISOString(),
    }],
    allowed_mentions: { parse: [] },
  });
  shopState.lastCampaignAt[guildId] = new Date().toISOString();
  await saveShopState();
}
  ...
  try {
    await discordApi(`/applications/${applicationId}/guilds/${guildId}/commands`, {
      ...
      body: JSON.stringify([
        ...
        { name: "ping", description: "Botun Discord API ve Gateway gecikmesini ölçer.", type: 1 },
...
async function handleSlashCommand(interaction: JsonRecord): Promise<void> {
  ...
  if (command === "ping") {
    ...
    try {
      await discordApi("/gateway");
      const apiPing = Date.now() - apiStarted;
      const gatewayText = gatewayPingMs >= 0 ? `${gatewayPingMs} ms` : "Ölçülüyor…";
      await interactionReply(
        interaction,
        `🏓 **Pong!**\n> Discord API: **${apiPing} ms**\n> Gateway: **${gatewayText}**`,
      );
    } catch (error) {
      logger.warn({ err: error }, "Ping testi başarısız");
      await interactionReply(interaction, "❌ Discord bağlantısı test edilemedi. Gateway bağlantısını kontrol et.", [], true);
    }
    return;
  }
  ...
  if (command === "coin-ekle") {
    ...
    await setShopCoins(guildId, targetId, before + added);
    await interactionReply(interaction, `✅ <@${targetId}> kullanıcısına **${added.toLocaleString("tr-TR")} coin** eklendi. Yeni bakiye: **${(before + added).toLocaleString("tr-TR")} coin**.`, [], true);
    return;
  }
...
async function handleComponent(interaction: JsonRecord): Promise<void> {
  ...
  if (action === "shop_confirm_role") {
    ...
    const price = shopEffectivePrice(guildId, shopPriceForRole(role));
    ...
    await interactionUpdate(interaction, `✅ **Sipariş oluşturuldu!**\n\n**Ürün:** ${String(role.name)}\n**Tutar:** ${price.toLocaleString("tr-TR")} Coin\n**Sipariş ID:** \`${order.id}\`\n\n📩 FRArda'ya sipariş detayları gönderildi.`, [row(button("shop_back", "↩️ Mağazaya Dön", 2))], [shopPersonalHomeEmbed(guildId, user, getShopCoins(guildId, String(user.id ?? "")))]);
    return;
  }
  ...
  if (action === "shop_confirm_special") {
    ...
    await interactionUpdate(interaction, `✅ **Özel rol siparişin oluşturuldu!**\n\n**Tutar:** ${price.toLocaleString("tr-TR")} Coin\n**Sipariş ID:** \`${order.id}\`\n\n📩 FRArda'ya kullanıcı ve sipariş detayların gönderildi.`, [row(button("shop_back", "↩️ Mağazaya Dön", 2))], [shopPersonalHomeEmbed(guildId, user, getShopCoins(guildId, String(user.id ?? "")))]);
    return;
  }
  ...
  if (action === "shop_confirm_product") {
    ...
    await interactionUpdate(interaction, `✅ **Nitro siparişin oluşturuldu!**\n\n**Tutar:** ${price.toLocaleString("tr-TR")} Coin\n**Sipariş ID:** \`${order.id}\`\n\n📩 FRArda'ya sipariş detayların gönderildi.`, [row(button("shop_back", "↩️ Mağazaya Dön", 2))], [shopPersonalHomeEmbed(guildId, user, getShopCoins(guildId, String(user.id ?? "")))]);
    return;
  }
  ...
  if (customId === "shop_open") {
    ...
    await loadShopState();
    const coins = getShopCoins(guildId, String(user.id ?? ""));
    const personalComponents = [row(shopMenu()), row(button("shop_refresh", "🔄 Bakiyeyi Yenile", 2))];
    await interactionReply(interaction, "", personalComponents, true);
    await editInteractionOriginal(interaction, {
      embeds: [shopPersonalHomeEmbed(guildId, user, coins)],
  ...
  if (customId === "shop_refresh") {
    ...
    await loadShopState();
    const coins = getShopCoins(guildId, String(user.id ?? ""));
    await interactionReply(interaction, "", [row(shopMenu()), row(button("shop_refresh", "🔄 Bakiyeyi Yenile", 2))], true);
    await editInteractionOriginal(interaction, { embeds: [shopPersonalHomeEmbed(guildId, user, coins)], components: [row(shopMenu()), row(button("shop_refresh", "🔄 Bakiyeyi Yenile", 2))] });
    return;
  }
  ...
  if (customId === "shop_menu") {
    ...
    if (selected === "roles") {
      ...
      const options = roles.map((role) => ({ label: String(role.name).slice(0, 100), value: String(role.id), description: `Fiyat: ${shopEffectivePrice(guildId, shopPriceForRole(role)).toLocaleString("tr-TR")} coin${shopCampaignActive(guildId) ? " • %50 kampanya" : ""}`.slice(0, 100) }));
      ...
      await editInteractionOriginal(interaction, { embeds: shopRoleEmbeds(roles, guildId, coins), components: roleComponents });
      setTimeout(() => {
        void editInteractionOriginal(interaction, { content: "🛍️ İşlem yapılmadığı için kişisel mağaza ana menüsüne dönüldü.", embeds: [shopPersonalHomeEmbed(guildId, user, getShopCoins(guildId, String(user.id ?? "")))], components: [row(shopMenu()), row(button("shop_refresh", "🔄 Bakiyeyi Yenile", 2))] }).catch(() => undefined);
      }, 60_000);
      return;
    }
    ...
    if (selected === "buy-coins") {
      const text = "💰 Coin satın almak için FRArda ile iletişime geçebilirsin. İşlem manuel olarak yapılır.";
  ...
  if (customId === "shop_back") {
    ...
    const user = (interaction.member?.user ?? interaction.user ?? {}) as JsonRecord;
    const coins = getShopCoins(guildId, String(user.id ?? ""));
    await interactionUpdate(interaction, "", [row(shopMenu()), row(button("shop_refresh", "🔄 Bakiyeyi Yenile", 2))], [shopPersonalHomeEmbed(guildId, user, coins)]);
    return;
  }
...
async function handleMessage(message: JsonRecord): Promise<void> {
  ...
  // Yönetici/moderatör düz metin olarak da `shop` yazarsa slash komutuyla aynı mağaza açılır.
  if (isModerator && messageContent.toLocaleLowerCase("tr-TR") === "shop") {
    const published = await publishShopMenu(guildId);
    if (published) {
      await deleteMessage(String(message.channel_id ?? ""), String(message.id ?? "")).catch(() => undefined);
    }
    return;
  }
...
function connectGateway(token: string): void {
  ...
  socket.onmessage = (event) => {
    try {
      ...
      if (payload.op === 11) {
        if (heartbeatSentAt > 0) gatewayPingMs = Date.now() - heartbeatSentAt;
        return;
      }
