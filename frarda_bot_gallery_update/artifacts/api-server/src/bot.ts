...
function isShopRole(role: JsonRecord): boolean {
  ...
  // Görseldeki FR | hiyerarşisini temel alır. Yeni bir FR | rolü eklenince
  // mağazada da otomatik görünür; Discord'daki gerçek rol sırası korunur.
...
function shopEffectivePrice(guildId: string, basePrice: number): number {
  return shopCampaignActive(guildId) ? Math.max(1, Math.floor(basePrice * 0.9)) : basePrice;
}
...
function shopPublicEmbed(coins: number | null = null): JsonRecord {
  const campaign = coins !== null ? shopCampaignActive("__preview__") : false;
  return {
    ...
    footer: { text: "FR FAMILY SHOP • Kişisel menü • Siparişler manuel onaylanır" },
...
function shopRolesEmbed(roles: JsonRecord[], guildId: string, coins: number): JsonRecord {
  ...
  return {
    ...
    description: [
      ...
      campaign ? "🏷️ **Kampanya aktif:** seçili ürünlerde %10 indirim" : "",
      "",
...
function shopMenu(): JsonRecord {
  return {
    ...
    options: [
      ...
      { label: "Join Satın Al", value: "join", description: "Join hizmeti için FRArda'ya ulaş", emoji: { name: "📩" } },
    ],
  };
}
...
async function handleShopSlash(interaction: JsonRecord, guildId: string): Promise<void> {
  ...
  await sendMessage(String(channel.id), {
    embeds: [shopPublicEmbed()],
    components: [row(shopMenu())],
    allowed_mentions: { parse: [] },
  });
  await interactionReply(interaction, "✅ FR Family Shop menüsü #mağaza kanalına gönderildi.", [], true);
}
...
async function runCampaignForGuild(guildId: string): Promise<void> {
  ...
  await sendMessage(String(channel.id), {
    embeds: [{
      ...
      description: "5 günlük kampanya dönemi başladı! Mağazadaki seçili ürünlerde küçük indirimler seni bekliyor.",
      color: 0xf59e0b,
      image: process.env.SHOP_CAMPAIGN_IMAGE_URL ? { url: process.env.SHOP_CAMPAIGN_IMAGE_URL } : undefined,
      fields: [
        { name: "🏷️ Kampanya", value: "Seçili ürünlerde **%10 indirim**", inline: true },
...
async function handleComponent(interaction: JsonRecord): Promise<void> {
  ...
  if (action === "shop_confirm_role") {
    ...
    await interactionUpdate(interaction, `✅ **Sipariş oluşturuldu!**\n\n**Ürün:** ${String(role.name)}\n**Tutar:** ${price.toLocaleString("tr-TR")} Coin\n**Sipariş ID:** \`${order.id}\`\n\n📩 FRArda'ya sipariş detayları gönderildi.`, [row(button("shop_back", "↩️ Mağazaya Dön", 2))], [shopPublicEmbed(getShopCoins(guildId, String(user.id ?? "")))]);
    return;
  }
  ...
  if (action === "shop_confirm_special") {
    ...
    await interactionUpdate(interaction, `✅ **Özel rol siparişin oluşturuldu!**\n\n**Tutar:** ${price.toLocaleString("tr-TR")} Coin\n**Sipariş ID:** \`${order.id}\`\n\n📩 FRArda'ya kullanıcı ve sipariş detayların gönderildi.`, [row(button("shop_back", "↩️ Mağazaya Dön", 2))], [shopPublicEmbed(getShopCoins(guildId, String(user.id ?? "")))]);
    return;
  }
  ...
  if (action === "shop_confirm_product") {
    ...
    await interactionUpdate(interaction, `✅ **Nitro siparişin oluşturuldu!**\n\n**Tutar:** ${price.toLocaleString("tr-TR")} Coin\n**Sipariş ID:** \`${order.id}\`\n\n📩 FRArda'ya sipariş detayların gönderildi.`, [row(button("shop_back", "↩️ Mağazaya Dön", 2))], [shopPublicEmbed(getShopCoins(guildId, String(user.id ?? "")))]);
    return;
  }
  ...
  if (customId === "shop_menu") {
    ...
    if (selected === "roles") {
      ...
      const options = roles.map((role) => ({ label: String(role.name).slice(0, 100), value: String(role.id), description: `Fiyat: ${shopEffectivePrice(guildId, shopPriceForRole(role)).toLocaleString("tr-TR")} coin${shopCampaignActive(guildId) ? " • %10 kampanya" : ""}`.slice(0, 100) }));
      ...
      setTimeout(() => {
        void editInteractionOriginal(interaction, { content: "🛍️ İşlem yapılmadığı için mağaza ana menüsüne dönüldü.", embeds: [shopPublicEmbed(coins)], components: [row(shopMenu())] }).catch(() => undefined);
      }, 60_000);
      return;
    }
