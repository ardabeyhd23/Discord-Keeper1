  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <div className="side-links">
        ...
        <button className={activeView === "support" ? "side-link selected" : "side-link"} onClick={() => setActiveView("support")}>◌ <span>Destek Merkezi</span></button>
      ...
      {activeView === "shop" && <><div className="sidebar-category-title">KATEGORİLER</div>
        <div className="sidebar-categories">
          {categories.map((item) => <button key={item} className={category === item ? "category-link active" : "category-link"} onClick={() => { setCategory(item); setActiveView("shop"); }}><span>{item === "Tümü" ? "✦" : item === "Üst Roller" ? "♛" : item === "Özel Rol" ? "◇" : "◈"}</span><b>{item}</b><small>{item === "Tümü" ? products.length : products.filter((product) => product.category === item).length}</small></button>)}
        </div></>}
    ...
    {activeView === "shop" && <section className="hero">
     ...
     {activeView === "shop" && <nav className="shop-nav"><div className="tabs">{categories.map((item) => <button className={category === item ? "tab active" : "tab"} onClick={() => setCategory(item)} key={item}>{item}</button>)}</div></nav>}
    ...
    {activeView === "shop" && <section className="product-grid">{visible.map((p) => <article className={p.featured ? "product-card featured" : "product-card"} key={p.id}><div className="card-top"><ProductIcon icon={p.icon}/>{p.featured && <span className="badge">ÖNE ÇIKAN</span>}</div><p className="product-category">{p.category}</p><h3>{p.name}</h3><p className="product-description">{p.description}</p><div className="card-bottom"><strong>{money(p.price)}</strong><button onClick={() => setSelected(p)}>İncele <span>↗</span></button></div></article>)}</section>}
     {activeView === "shop" && <section className="shop-benefits"><div><span>♢</span><p><b>Güvenli ödeme</b><small>Tüm işlemler güvenli ve şifreli.</small></p></div><div><span>ϟ</span><p><b>Anında teslimat</b><small>Ödemeden sonra hızlı işlem.</small></p></div><div><span>♧</span><p><b>7/24 destek</b><small>Her zaman yanındayız.</small></p></div></section>}
    ...
    {activeView === "coins" && <section className="panel-list">{coinHistory.length ? coinHistory.map((e) => <article className="history-row" key={e.id}><div><b>{e.reason}</b><small>{new Date(e.createdAt).toLocaleString("tr-TR")}</small></div><strong className={e.amount > 0 ? "coin-plus" : "coin-minus"}>{e.amount > 0 ? "+" : ""}{e.amount.toLocaleString("tr-TR")} Coin<small> Bakiye: {e.balance.toLocaleString("tr-TR")}</small></strong></article>) : <div className="empty-panel">Henüz coin hareketin yok.</div>}</section>}
     {activeView === "support" && <section className="support-chat">
       <aside className="ticket-list">
         <div className="chat-list-head"><div><p className="eyebrow">FR FAMILY DESTEK</p><h2>Mesajlar</h2></div><span className="support-icon">✦</span></div>
         <button className="new-ticket-button" onClick={() => setSelectedTicketId("new")}>＋ Yeni destek talebi</button>
         {tickets.map((t) => <button className={selectedTicketId === t.id ? "ticket-item active" : "ticket-item"} key={t.id} onClick={() => setSelectedTicketId(t.id)}><span className="ticket-avatar">FR</span><span><b>{t.subject}</b><small>{t.messages?.length || 0} mesaj · {t.status === "acik" ? "Açık" : t.status === "kapali" ? "Kapalı" : "Bekliyor"}</small></span><i>›</i></button>)}
       ...
       <div className="chat-window">
         {selectedTicketId === "new" || (!tickets.length && !selectedTicketId) ? <div className="new-chat">
           <span className="large-chat-icon">✦</span><p className="eyebrow">FR FAMILY DESTEK MERKEZİ</p><h2>Yeni bir konuşma başlat</h2><p>Konunu yaz, destek ekibi buradan yanıtlasın. Açtığın talepler ve gönderdiğin her mesaj FrArda'ya da iletilir.</p>
           <div className="new-ticket-form"><input value={supportSubject} onChange={(e) => setSupportSubject(e.target.value)} placeholder="Örn. Siparişim teslim edilmedi" /><button className="primary" onClick={createTicket}>Sohbeti başlat <span>→</span></button></div>
         </div> : (() => {
