...
export default function App() {
  ...
  useEffect(() => {
    ...
    const setupActivity = async () => {
      ...
      if (!discordClientId) {
        setActivityError("Discord Activity için VITE_DISCORD_CLIENT_ID eksik.");
        return;
      }
      ...
      try {
        ...
        if (!response.ok) throw new Error("activity_auth");
        ...
        console.error(error);
        if (!cancelled) setActivityError("Discord hesabı doğrulanamadı. Activity ayarlarını kontrol et.");
      }
    };
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (response.status === 401) { if (isDiscordActivity) { setActivityError("Discord oturumu alınamadı. Activity yetkilendirmesini yeniden dene."); return; } const auth = await response.json(); window.location.href = auth.loginUrl; return; }
  ...
  const buy = async () => {
    ...
    try {
      if (api) {
        ...
        if (res.status === 401) { if (isDiscordActivity) { setToast("Discord oturumu hazır değil. Activity’yi yeniden aç."); return; } const auth = await res.json(); window.location.href = auth.loginUrl; return; }
  ...
  return <main className="app-shell">
    {activityError && <div className="toast">⚠️ {activityError}</div>}
