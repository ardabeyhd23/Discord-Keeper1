...
function CoinMark() { return <span className="coin-mark" aria-label="FR Coin">FR</span>; }
...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    <nav className="shop-nav"><div className="tabs">{categories.map((item) => <button className={category === item ? "tab active" : "tab"} onClick={() => setCategory(item)} key={item}>{item}</button>)}</div><label className="search">⌕<input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Ürün ara..." /></label></nav>
